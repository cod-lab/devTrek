...
exports.readController = async (res, id, pg) => {
    ...
    try {
        ...
        if(pg === 'index') {        // [GET], index pg
            ...
            articles.sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt));    // sort objects inside array in descending order of articles[i].id
            ...
            // console.log('article',article,'id',id,typeof id);
...
exports.writeController = async (req, res, event, pg) => {
    ...
    // article.sortingDate = new Date();
    // article.displayDate = article.sortingDate.toLocaleDateString();
    ...
    try {
        ...
        if(!id) {        // [POST], new article
            ...
            // id = Math.trunc((Math.random() * 87365012)) +''+ Math.trunc((Math.random() * 10293458));
            // article.createdAt = new Date();
            const displayDate = article.createdAt.toLocaleDateString();
            const chkDate = /^\d\//;  // check string starts with '[1-9]/' i.e. date 1 - 9 only
            // if(chkDate.test(article.displayDate)) article.displayDate = '0'+article.displayDate;
            article.displayDate = chkDate.test(displayDate) ? '0'+displayDate : displayDate;
            article.likes = 0;
            articles.push(article);
        } else {        // [PUT], update article
            article.id = id;
            // const oldArticle = articles.find(article => article.id === id);
